 ...
 * 
 */
class NoCaptchaValidator extends Validator
{
    ...
    public function init()
    {
        ...
        if(!isset($this->secret)){
             throw new InvalidConfigException('Required secret for communicate with google server');
        }
    ...
    protected function validateValue($value)
    {
        ...
        $request = self::SITE_VERIFY_URL . '?' . http_build_query(
            [
                'secret' => $this->secret,
                'response' => $value,
                'remoteip' => Yii::$app->request->userIP // ip address of client
            ]
        );
